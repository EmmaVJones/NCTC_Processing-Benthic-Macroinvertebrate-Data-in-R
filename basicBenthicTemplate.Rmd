...
bugData <- read_excel("data/benthicResults.xlsx") %>%
...
Here is a basic plot of Virginia Stream Condition Index scores for the input dataset. The function the plot is built with can handle more samples if you format your data to match  in input data template. You can also go in and alter the code to change SCI threshold to match your benthic cutoff.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Dissolved Oxygen in `r prettyStreamName` ranged from `r min(envData$'DO (mg/L)', na.rm=TRUE)` -  `r max(envData$'DO (mg/L)', na.rm=TRUE)` mg/L.  The median dissolved oxygen value was `r median(envData$'DO (mg/L)', na.rm=TRUE)`mg/L (n=`r nrow(envData[!is.na(envData$'DO (mg/L)'), ])`) (Figure 7). 
...
Total nitrogen ranged from `r min(envData$'TN (mg/L)', na.rm=TRUE)` -  `r max(envData$'TN (mg/L)', na.rm=TRUE)` mg/L with a median of `r median(envData$'TN (mg/L)', na.rm=TRUE)` mg/L (n= `r nrow(envData[!is.na(envData$'TN (mg/L)'), ])`) (Figure 9). 
...
Total dissolved solids measurements in `r prettyStreamName` ranged from `r min(envData$"TDS (mg/L)", na.rm=TRUE)` -  `r max(envData$"TDS (mg/L)", na.rm=TRUE)` mg/L (Figure 14). The median TDS value was `r median(envData$"TDS (mg/L)", na.rm=TRUE)` mg/L  (n=`r nrow(envData[!is.na(envData$"TDS (mg/L)"), ])`). 

```{r TDS, echo=FALSE,message=FALSE, fig.width=9, fig.height=6, fig.cap=paste("Figure 14: Describe TDS data here.")}
...
Specific Conductivity measurements in `r prettyStreamName` ranged from `r min(envData$"SpCond (uS/cm)", na.rm=TRUE)` -  `r max(envData$"SpCond (uS/cm)", na.rm=TRUE)` uS/cm (Figure 13). The median specific conductivity value was `r median(envData$"SpCond (uS/cm)", na.rm=TRUE)` uS/cm  (n=`r nrow(envData[!is.na(envData$"SpCond (uS/cm)"), ])`). 
...
If you are planning on distributing report as an HTML document, I recommend interactive plots with plotly. Below is an example with the total habitat scores. 

`r prettyStreamName` total habitat scores ranged from `r min(envData$'Total Habitat (unitless)', na.rm=TRUE)` -  `r max(envData$'Total Habitat (unitless)', na.rm=TRUE)` and the median value was `r median(envData$'Total Habitat (unitless)', na.rm=TRUE)`(n=`r nrow(envData[!is.na(envData$'Total Habitat (unitless)'), ])`) (Figure 9). 

```{r , echo=FALSE,message=FALSE, fig.width=9, fig.height=6, fig.cap=paste("Figure 10: Describe Total Habitat data here.")}
